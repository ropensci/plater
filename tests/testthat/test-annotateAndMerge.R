for (i in c(12, 24, 48, 96, 384)) {
   ...
   ################################################################################
   context("testing annotateAndMerge-annotateAndMerge()")
   ################################################################################
   nLetters <- ifelse(i == 12, 12, 24)
   
   test_that("annotateAndMerge works for complete valid data", {
   ...
   test_that("annotateAndMerge works for missing data from plate", {
   ...
   test_that(paste("annotateAndMerge works for missing data from",
   ...
   test_that("annotateAndMerge stops if wells missing from df", {
      filename <- paste0(path, "allWellIds.csv")
      complete <- data.frame(wells = getWellIds(i), d = letters[1:nLetters])
      complete$d <- as.character(complete$d)
      
      expect_that(annotateAndMerge(complete[1:(i-1), ], i, "wells", 
         filename, "values"),
         throws_error())
   })
   
   test_that(paste("annotateAndMerge stops if some wells are missing",
      ...
      expect_that(annotateAndMerge(complete, "wells", i,
         filename, "values"),
         throws_error())
   })
